    ...
    return (
        <div className="w-full max-container bg-slate-50">
            <div className="relative flex-1 flex justify-center items-center">
                ...
                <div className="flex gap-4 absolute -bottom-[5%] z-10">
                    <BannerSlider />
                </div>
            </div>
            <div className="w-full center mt-28 gap-x-4">
                {bannerContent.map((value) => (
                    <BannerContent image={value.image} title={value.title} />
                ))}
            </div>
            <div className="w-full">
                <img src="/image/banner-footer.png" alt="banner" className="w-full my-10" />
            </div>
        </div>
    );
}
